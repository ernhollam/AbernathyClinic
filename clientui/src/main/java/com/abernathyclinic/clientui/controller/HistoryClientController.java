...
public class HistoryClientController {
    ...
    public String showPatientHistory(@PathVariable("patientId") Integer patientId, Model model, RedirectAttributes redirectAttributes) {
    ...
    public String showUpdateNoteForm(@PathVariable("patientId") Integer patientId, @PathVariable("noteId") String noteId, Model model, RedirectAttributes redirectAttributes) {
        PatientBean patient = getPatientIfExists(patientId, redirectAttributes);
        if (patient == null) {
            return "patient/list";
        }
    ...
    public String updateNote(@PathVariable("patientId") Integer patientId, @PathVariable("noteId") String noteId, @Valid NoteBean note,
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("error", "Error while trying to update note " + note.getId() + ":\n" + exception.getMessage());
        }
        model.addAttribute("patHistory", historyProxy.getPatientHistory(patientId));
        model.addAttribute("patient", patient);
        return "redirect:/note/list";
    ...
    public String addNoteForPatient(@PathVariable("patientId") Integer patientId, @Valid @RequestBody NoteBean note, BindingResult result, Model model, RedirectAttributes redirectAttributes) {
        PatientBean patient = getPatientIfExists(patientId, redirectAttributes);
        if (patient == null) {
            return "patient/list";
        }
        ...
        if (!result.hasErrors()) {
            try {
                ...
                redirectAttributes.addFlashAttribute("success", "Note " + note.getContent() + " was successfully created.");
    ...
    public String deletePatient(@PathVariable("patientId") Integer patientId, @PathVariable("noteId") String noteId, Model model, RedirectAttributes redirectAttributes) {
        PatientBean patient = getPatientIfExists(patientId, redirectAttributes);
        if (patient == null) {
            return "patient/list";
        }
